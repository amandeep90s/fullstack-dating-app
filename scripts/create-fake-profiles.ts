...
async function createFakeProfiles() {
  ...
  for (let i = 0; i < fakeProfiles.length; i++) {
    ...
    try {
      ...
      if (existingAuthUser) {
        console.log(`⚠️ Auth user already exists for ${profile.full_name}, using existing...`);
        ...
        if (authError) {
          console.error(`❌ Error creating auth user for ${profile.full_name}:`, authError);
          continue;
        }
      ...
      if (existingProfile) {
        console.log(`⚠️ Profile already exists for ${profile.full_name}, updating...`);
        ...
        if (updateError) {
          console.error(`❌ Error updating profile for ${profile.full_name}:`, updateError);
          continue;
        }
      } else {
        ...
        if (profileError) {
          console.error(`❌ Error creating profile for ${profile.full_name}:`, profileError);
      ...
      console.error(`❌ Unexpected error creating profile for ${profile.full_name}:`, error);
    }
  }
